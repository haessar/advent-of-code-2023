...
def get_groups(line):
    return [Counter(grp)['#'] for grp in filter(None, line.split("."))]


for line in test_complete:
    assert get_groups(line.split(" ")[0]) == list(map(int, line.split(" ")[1].split(",")))
...
for line in tqdm(data):
    ...
    arrangements = 0
    for perm in distinct_permutations(candidates):
        perm = list(perm)
        test = ''
        for c in record:
            if c == "?":
